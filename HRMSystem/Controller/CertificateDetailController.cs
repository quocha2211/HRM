    ...
    public class CertificateDetailController : IucControlController
    {
        ...
        private void MasterForm_DeleteButtonClick(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    var model = context.ChungChis.Find(Convert.ToInt32(masterForm.GetPrimaryKey("MaChucVu")));
        ...
        private void MasterForm_EditButtonClick(object sender, EventArgs e)
        {
            try
            {
                if (masterForm == null)
                    return;
                InitialDetailPage(masterForm.GetPrimaryKey("MaCC"));
            }
            catch (Exception ex) { SQLiteHelper.SaveToLog(ex.Message, "ChucVuController", ex.ToString()); }
            finally { clsCommon.CloseWaitingForm(); }
        }
        ...
        private void InitialDetailPage(string pKey)
        {
            try
            {
                ...
                if (detailForm != null)
                    detailForm.Dispose();
                detailForm = new frmCertificateDetail() { Dock = DockStyle.Fill };
