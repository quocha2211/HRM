    ...
    public class DaoTaoController : IucControlController
    {
        ...
        private void MasterForm_DeleteButtonClick(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    var model = context.HopDongs.Find(Convert.ToInt32(masterForm.GetPrimaryKey("MaQTDT")));
        ...
        private void MasterForm_EditButtonClick(object sender, EventArgs e)
        {
            try
            {
                ...
                InitialDetailPage(masterForm.GetPrimaryKey("MaQTDT"));
            }
            catch (Exception ex) { SQLiteHelper.SaveToLog(ex.Message, "SalaryScaleController", ex.ToString()); }
            finally { clsCommon.CloseWaitingForm(); }
        }
        ...
        private void InitialDetailPage(string pKey)
        {
            try
            {
                ...
                if (detailForm != null)
                    detailForm.Dispose();
                detailForm = new frmDaoTao() { Dock = DockStyle.Fill };
                detailForm.MaNV = Convert.ToInt32( masterForm.GetPrimaryKey("MaNV"));
                detailForm.MaQTDT = pKey;
            ...
            try
            {
                using (var context = new AppDbContext())
                {
                    ...
                    var query = (from nv in context.NhanViens
                                join xl in context.QuaTrinhDaoTaos on nv.MaNV equals xl.MaNV into hopDongGroup
                    ...
                    clsCommon.OpenWaitingForm(View);
                    masterForm.SetTitle("Quản lý Quá Trình Đào Tạo");
                    masterForm.SetDataSource(query, clsInitialGridColumn.InitialQuaTrinhDaoTao());
