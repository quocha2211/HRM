    ...
    public class EmployeeTranferController : IucControlController
    {
        ...
        private void InitialDetailPage(string pKey)
        {
            try
            {
                ...
                if (detailForm != null)
                    detailForm.Dispose();
                detailForm = new frmEmployeeTranfer() { Dock = DockStyle.Fill };
        ...
        private void LoadData()
        {
            using (var context = new AppDbContext())
            {
                var query = (from xl in context.DieuChuyenCongTacs
                             join nv in context.NhanViens on xl.MaNV equals nv.MaNV
                             join pb in context.PhongBans on xl.MaPB equals pb.MaPB
                             join bl in context.BangLuongs on xl.MaNV equals bl.MaNV
                             join hd in context.HopDongs on xl.MaNV equals hd.MaNV
                             select new
                             {
                                 xl.MaCCT,
                                 nv.TenNV,
